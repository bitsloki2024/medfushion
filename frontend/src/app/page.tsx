  ...
  loading: () => (
    <div className="w-full h-full flex items-center justify-center" style={{ background: '#000814' }}>
      <div className="text-center">
        ...
        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.55rem', color: '#3a5a78', letterSpacing: '0.28em', textTransform: 'uppercase' }}>Initializing Globe</p>
      </div>
    </div>
  ),
});
...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<'surveillance' | 'classification' | 'genomics' | 'therapeutics' | 'visual'>('surveillance');
  ...
  const TABS: { key: typeof activeTab; label: string; icon: string }[] = [
    ...
    { key: 'visual',          label: 'Visual Intel',    icon: '📊' },
  ];
  ...
  return (
    <div style={{ height: '100vh', width: '100vw', display: 'flex', flexDirection: 'column', background: '#000814', overflow: 'hidden', fontFamily: 'Inter, -apple-system, sans-serif' }}>
      ...
      {/* ── Header ── */}
      <header style={{ flexShrink: 0, zIndex: 30, borderBottom: '1px solid rgba(0,120,180,0.12)', background: 'rgba(0,6,18,0.94)', backdropFilter: 'blur(24px)', padding: '0 1.5rem', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        {/* Brand */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
          ...
          <div>
            <div style={{ fontSize: '1rem', fontWeight: 600, letterSpacing: '0.1em', color: '#c8e8f8' }}>CosmoSentinel</div>
            <div style={{ fontSize: '0.45rem', color: '#2e4a62', letterSpacing: '0.25em', textTransform: 'uppercase', marginTop: 1 }}>Global Disease Intelligence</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '0.4rem' }}>
          {(Object.entries(DISEASE_META) as [DiseaseKey, typeof meta][]).map(([key, m]) => (
            <button key={key} onClick={() => { setDisease(key); setSelectedCountry(null); }}
              style={{ padding: '4px 10px', borderRadius: 20, border: `1px solid ${disease === key ? m.color : 'rgba(255,255,255,0.08)'}`, background: disease === key ? `${m.color}20` : 'transparent', color: disease === key ? m.color : '#4a6785', fontSize: '0.6rem', fontWeight: 500, letterSpacing: '0.06em', cursor: 'pointer', transition: 'all 0.2s ease', fontFamily: 'Inter, sans-serif' }}>
              {m.icon} {m.label}
            </button>
          ))}
        </div>
        ...
        {/* Status */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', fontSize: '0.58rem', color: '#2e4a62' }}>
          ...
          <AnimatePresence>
            {!selectedCountry && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                <motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 3, repeat: Infinity }} style={{ fontSize: '0.52rem', letterSpacing: '0.36em', color: 'rgba(0,160,220,0.7)', textTransform: 'uppercase', marginBottom: '0.5rem' }}>
                  Select a region to begin
                </motion.div>
                <div style={{ fontSize: '0.6rem', color: '#1e3040', letterSpacing: '0.12em' }}>
                  {countriesAffected} countries · {formatNumber(totalCases)} cases · {activeAlerts} high-risk alerts
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <AnimatePresence>
            {selectedCountry && (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                ...
                <span style={{ color: '#80c8e8', fontSize: '0.65rem', fontWeight: 500, letterSpacing: '0.08em' }}>{selectedCountry.country}</span>
                <button onClick={() => setSelectedCountry(null)} style={{ color: '#2e4a62', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.7rem', marginLeft: 4 }}>✕</button>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {selectedCountry && (
            <div style={{ position: 'absolute', bottom: 16, left: 16, zIndex: 10, background: 'rgba(0,12,30,0.9)', border: '1px solid rgba(0,120,180,0.15)', borderRadius: 10, padding: '10px 14px', backdropFilter: 'blur(16px)', minWidth: 200 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: spreadMode ? 8 : 0 }}>
                <span style={{ fontSize: '0.52rem', color: '#3a5a78', letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 500 }}>Spread Sim</span>
              ...
              <div style={{ flexShrink: 0, display: 'flex', borderBottom: '1px solid rgba(0,90,140,0.12)', background: 'rgba(0,6,20,0.8)', padding: '0 0.5rem' }}>
                {TABS.map(tab => (
                  <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                    style={{ flex: 1, padding: '10px 4px', fontSize: '0.55rem', fontWeight: activeTab === tab.key ? 500 : 400, letterSpacing: '0.08em', textTransform: 'uppercase', border: 'none', background: 'none', color: activeTab === tab.key ? '#60b8dc' : '#2e4a62', borderBottom: `2px solid ${activeTab === tab.key ? '#60b8dc' : 'transparent'}`, cursor: 'pointer', fontFamily: 'Inter, sans-serif', transition: 'all 0.2s ease', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
      ...
      <footer style={{ flexShrink: 0, borderTop: '1px solid rgba(0,80,120,0.12)', background: 'rgba(0,6,18,0.9)', backdropFilter: 'blur(20px)', padding: '0 1.5rem', height: 40, display: 'flex', alignItems: 'center', gap: 0 }}>
        {[
          ...
          <div key={s.label} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, padding: '0 12px', borderRight: i < 4 ? '1px solid rgba(0,80,120,0.1)' : 'none' }}>
            <div>
              <div style={{ fontSize: '0.8rem', fontWeight: 300, color: s.color, letterSpacing: '-0.01em' }}>{typeof s.value === 'number' ? s.value.toLocaleString() : s.value}</div>
              <div style={{ fontSize: '0.45rem', color: '#1e3040', letterSpacing: '0.2em', textTransform: 'uppercase', marginTop: 1 }}>{s.label}</div>
            </div>
          </div>
        ))}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 'auto', paddingLeft: 12 }}>
          ...
          <span style={{ fontSize: '0.5rem', color: '#22c55e', letterSpacing: '0.2em', fontWeight: 500 }}>LIVE SURVEILLANCE</span>
        </div>
      </footer>
    </div>
  );
